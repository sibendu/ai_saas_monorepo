...
1. Do not launch VS Code, Cursor, or any other editor automatically.
2. Report the `{spec_file}` path so the user can open it themselves if they want to review the Suggested Review Order.
3. Display a summary in conversation output, including:
   ...
   - A note with the `{spec_file}` path. Mention that `{spec_file}` now contains a Suggested Review Order.
   - **Navigation tip:** "Ctrl+click (Cmd+click on macOS) the links in the Suggested Review Order to jump to each stop."
4. Offer to push and/or create a pull request.
